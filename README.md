...
| ⚙️ 관리 | ManagementView | 가입 신청 승인/거절, 멤버 역할 변경, 추방 (팀장 전용) |
...
| # | 발견 | 원인 | 해결 | 결과 |
|---|------|------|------|------|
| 1 | **로그인 후 홈 화면 기본 팀 선택 기준 부재** | 다중 팀 소속 구조에서 로그인 시 어느 팀을 기본으로 표시할지 기준이 없어 매번 직접 선택해야 함 | 로그인 성공 직후 `getfavoriteteam.php` 단일 API로 즐겨찾기 팀 조회 후 분기 처리 — teamId > 0이면 TeamHomeView 바로 진입, 0이면 TeamSelectView로 이동 | 재접속 시 마지막 즐겨찾기 팀으로 즉시 진입. 로그인 → 홈 진입 API 호출 **2회 → 1회** |
| 2 | **팀 전환 후 이전 팀 데이터 덮어씌우기** | 팀 전환 후 `TeamHomeView`가 `onAppear`를 재실행하면서 전환 전 팀 ID로 다시 데이터 조회 | `TeamSwitchActive` Boolean 플래그 도입 — 전환 직후 `true`이면 `onAppear` 재조회 건너뛰고 `false`로 초기화, 다음 `onAppear`부터 정상 실행 | 팀 전환 후 전환된 팀 데이터가 덮어씌워지지 않고 올바르게 유지 |
| 3 | **댓글·공지 수정/삭제 권한 — 서버 검증 없이 UI만 제어** | 버튼을 작성자 본인에게만 표시했지만 API 요청 자체는 누구나 보낼 수 있어 권한이 클라이언트 단에서만 보장됨 | `deleteComment.php` 등 PHP 엔드포인트에서 전달된 `user_id`를 DB 작성자 ID와 대조 검증, 불일치 시 `"unauthorized"` 반환 후 처리 중단 | 클라이언트 UI 제어 + 서버 검증 이중 적용으로 무단 수정/삭제 완전 차단 |
